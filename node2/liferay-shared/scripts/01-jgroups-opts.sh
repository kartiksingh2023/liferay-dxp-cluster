...
if ! grep -q "jgroups.tcpping.initial_hosts" "$SETENV"; then
  echo '' >> "$SETENV"
  echo 'CATALINA_OPTS="$CATALINA_OPTS -Djgroups.bind_addr=liferay-node2"' >> "$SETENV"
  echo 'CATALINA_OPTS="$CATALINA_OPTS -Djgroups.tcpping.initial_hosts=liferay-node1[7800],liferay-node2[7801]"' >> "$SETENV"
  echo "export CATALINA_OPTS" >> "$SETENV"
  echo "[cluster-init] JGroups opts appended to setenv.sh (Node 2)"
else
